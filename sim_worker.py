...
async def close_position_if_triggered(conn, *, pos: Dict[str, Any], price: float, reason: str) -> bool:
    ...
    row = await conn.fetchrow(
        """
        SELECT id, anon_id, symbol, side, entry_price, qty, margin_usd
        FROM sim_positions
    ...
    credit = margin + pnl
    if reason == "liq":
        # At liquidation price, pnl should be approx -margin. Be defensive and never credit negative.
...
async def tick_once(session: aiohttp.ClientSession) -> None:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT id, anon_id, symbol, side, tp_price, sl_price, liq_price
            FROM sim_positions
            WHERE is_open=TRUE
        ...
        for r in rows:
            ...
            price = await binance_futures_price(session, symbol)
            if price is None:
                continue
